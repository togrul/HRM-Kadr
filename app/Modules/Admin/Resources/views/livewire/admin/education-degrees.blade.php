    ...
    <div class="flex flex-col items-center justify-between sm:flex-row filter bg-white py-2 px-2 rounded-xl">
        <div class="flex items-center justify-center space-x-2 action-section">
            <x-button class="space-x-2" mode="primary" wire:click.prevent="openCrud()">
                ...
                <span>{{ __('admin::references.buttons.add_degree') }}</span>
            </x-button>
        </div>
    </div>
    ...
    @if($isAdded)
        <div wire:transition class="flex border border-gray-300 rounded-md bg-slate-50 relative px-3 py-2 my-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 mt-4 w-full">
                ...
                <div class="flex flex-col">
                    <x-label for="form.title_az">{{ __('admin::references.fields.name') }}</x-label>
                    <x-livewire-input mode="default" name="form.title_az" wire:model="form.title_az"></x-livewire-input>
                    @error('form.title_az')
                        <x-validation> {{ $message }} </x-validation>
                    @enderror
                </div>
                <div class="flex flex-col">
                    <x-label for="form.title_en">{{ __('admin::references.fields.name') }} (EN)</x-label>
                    <x-livewire-input mode="default" name="form.title_en" wire:model="form.title_en"></x-livewire-input>
                </div>
                <div class="flex flex-col">
                    <x-label for="form.title_ru">{{ __('admin::references.fields.name') }} (RU)</x-label>
                    <x-livewire-input mode="default" name="form.title_ru" wire:model="form.title_ru"></x-livewire-input>
                </div>
                            ...
                            <tr>
                                ...
                                <x-table.td>
                                    <div class="flex flex-col space-y-1">
                                        <span class="text-sm font-medium">
                                            <b>AZ</b> - {{ $degree->title_az }}
                                        </span>
                                        <span class="text-sm font-medium">
                                            <b>EN</b> - {{ $degree->title_en }}
                                        </span>
                                        <span class="text-sm font-medium">
                                            <b>RU</b> - {{ $degree->title_ru }}
                                        </span>
                                    </div>
                                </x-table.td>
                                ...
                                <x-table.td :isButton="true" width="100">
                                    <div class="flex items-center space-x-2">
                                        <button
                                            wire:click.prevent="openCrud({{ $degree->id }})"
                                        ...
                                        <button
                                            wire:click.prevent = "deleteModel({{ $degree->id }})"
