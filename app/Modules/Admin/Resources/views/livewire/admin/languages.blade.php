    ...
    <div class="flex flex-col items-center justify-between sm:flex-row filter bg-white py-2 px-2 rounded-xl">
        <div class="flex items-center justify-center space-x-2 action-section">
            <x-button class="space-x-2" mode="primary" wire:click.prevent="openCrud()">
                ...
                <span>{{ __('admin::languages.actions.add') }}</span>
            </x-button>
        </div>
    </div>
    ...
    @if($isAdded)
        <div wire:transition class="flex border border-gray-300 rounded-md bg-slate-50 relative px-3 py-2 my-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 mt-4 w-full">
                <div class="flex flex-col">
                    <x-label for="form.id">{{ __('admin::languages.fields.id') }}</x-label>
                ...
                <div class="flex flex-col">
                    <x-label for="form.name">{{ __('admin::languages.fields.name') }}</x-label>
                ...
                <div class="flex items-end">
                    <x-modal-button mode="black">{{ __('admin::languages.actions.save') }}</x-modal-button>
                </div>
            </div>
        </div>
    @endif
        ...
        <div class="relative min-h-[300px] -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                <div class="overflow-visible">
                    <x-table.tbl :headers="[__('admin::languages.fields.id'), __('admin::languages.fields.name'), __('admin::languages.table.actions')]">
                            ...
                            <tr>
                                ...
                                <x-table.td :isButton="true" width="100">
                                    <div class="flex items-center space-x-2">
                                        <button
                                            wire:click.prevent="openCrud({{ $lang->id }})"
                                        ...
                                        <button
                                            wire:click.prevent = "deleteModel({{ $lang->id }})"
