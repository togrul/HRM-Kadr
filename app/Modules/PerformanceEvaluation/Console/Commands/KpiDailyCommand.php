...
class KpiDailyCommand extends Command
{
    ...
    protected $description = 'Close KPI cards of people who left or moved, auto-accept overdue agreements, send deadline reminders and escalations, scale cards for long leave, refill KPIs from HRM data, warn on red-zone forecasts, chase actuals and check-ins, send digests';

    public function handle(ScorecardLifecycleService $lifecycle, InternalKpiMetrics $metrics, KpiNotificationDelivery $delivery): int
    {
        ...
        $deadlines['red_zone'] = $lifecycle->notifyRedZone();
        $deadlines = [...$deadlines, ...$lifecycle->remindActualsAndCheckins()];
        $deadlines['digests'] = $delivery->sendDigests();
