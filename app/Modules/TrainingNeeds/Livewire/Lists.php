    ...
    public function getRowsProperty()
    {
        ...
        return match ($this->entity) {
            'plans' => TrainingPlanItem::query()
                ...
                ->latest('id')
                ->paginate(12, pageName: 'training-plan-items-page'),
            'sessions' => TrainingSession::query()
                ->when($search !== '', function ($query) use ($search) {
                    $query->where(function ($inner) use ($search) {
                ...
                ->paginate(12, pageName: 'training-sessions-page'),
            'deliveries' => TrainingDeliveryRecord::query()
                ->when($search !== '', function ($query) use ($search) {
                    $query->where(function ($inner) use ($search) {
                ...
                ->latest('completed_at')
                ->paginate(12, pageName: 'training-deliveries-page'),
            default => TrainingNeedItem::query()
                ->when($search !== '', function ($query) use ($search) {
                    $query->where(function ($inner) use ($search) {
                ...
                ->latest('id')
                ->paginate(12, pageName: 'training-needs-page'),
        };
    }
