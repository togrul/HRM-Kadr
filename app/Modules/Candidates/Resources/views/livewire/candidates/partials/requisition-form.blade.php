<div class="space-y-6">
    ...
    <div class="grid gap-4 lg:grid-cols-4">
        ...
        <div class="flex flex-col">
            <x-ui.select-dropdown
                :label="__('candidates::recruitment.labels.employment_type')"
                placeholder="---"
                mode="gray"
                class="w-full"
                wire:model.live="form.employment_type"
                :model="$this->recruitmentEmploymentTypeOptions()"
            />
            @error('form.employment_type') <x-validation>{{ $message }}</x-validation> @enderror
        </div>
    ...
    <div class="grid gap-4 lg:grid-cols-3">
        <div class="flex flex-col">
            <x-label for="form.hiring_reason">{{ __('candidates::recruitment.labels.hiring_reason') }}</x-label>
            <x-livewire-input mode="gray" name="form.hiring_reason" wire:model="form.hiring_reason"></x-livewire-input>
            @error('form.hiring_reason') <x-validation>{{ $message }}</x-validation> @enderror
        </div>

        <div class="flex flex-col">
            <x-label for="form.opens_at">{{ __('candidates::recruitment.labels.opens_at') }}</x-label>
            <x-pikaday-input mode="gray" name="form.opens_at" format="Y-MM-DD" wire:model.live="form.opens_at">
                <x-slot name="script">
                    $el.onchange = function () { @this.set('form.opens_at', $el.value); }
                </x-slot>
            </x-pikaday-input>
            @error('form.opens_at') <x-validation>{{ $message }}</x-validation> @enderror
        </div>
