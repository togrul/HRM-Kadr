                ...
                <div class="rounded-[20px] border border-zinc-200 bg-zinc-50 p-4">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div class="shrink-0 pt-1">
                            <label class="inline-flex items-center gap-3 rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700">
                    ...
                    <div class="mt-4 rounded-2xl border border-dashed border-zinc-200 bg-white p-4">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                            <div>
                                <div class="text-sm font-semibold text-zinc-900">{{ __('candidates::recruitment.labels.stage_document_upload') }}</div>
                                <div class="mt-1 text-sm text-zinc-500">{{ __('candidates::recruitment.labels.stage_document_upload_hint') }}</div>
                            </div>
                            <div class="flex flex-wrap items-center gap-3">
                                <input type="file" wire:model="uploadedDocumentFiles.{{ $item }}" multiple class="block text-sm text-zinc-500 file:mr-4 file:rounded-full file:border-0 file:bg-zinc-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-zinc-700 hover:file:bg-zinc-200" />
                                <button type="button" wire:click="uploadStageDocument('{{ $item }}')" class="inline-flex h-10 items-center rounded-xl bg-zinc-900 px-4 text-sm font-semibold text-white hover:bg-zinc-800">
                        ...
                        @if (! empty($this->currentStageDocumentsByKey[$item]))
                            <div class="mt-3 flex flex-wrap gap-2">
                                @foreach ($this->currentStageDocumentsByKey[$item] as $uploadedDocument)
                                    <a href="{{ route('candidates.documents.download', $uploadedDocument) }}" class="inline-flex rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs font-semibold text-zinc-700 hover:border-zinc-300">
