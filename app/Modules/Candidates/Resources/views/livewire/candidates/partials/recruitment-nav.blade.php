        ...
        <nav class="flex flex-wrap items-center gap-2">
            @foreach ($tabs as $tab)
                ...
                <a
                    ...
                    class="{{ $active ? 'border-slate-900 bg-slate-900 text-white shadow-card' : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-slate-300 hover:bg-white hover:text-slate-900' }} inline-flex items-center rounded-full border px-4 py-2 text-sm font-semibold transition"
                >
                    {{ $tab['label'] }}
                </a>
            @endforeach
