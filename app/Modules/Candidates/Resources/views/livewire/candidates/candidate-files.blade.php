<div class="flex flex-col space-y-5">
    ...
    <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-overlay">
        <div class="border-b border-zinc-200 bg-gradient-to-br from-zinc-50 via-white to-zinc-100 px-5 py-5">
            <div class="flex items-start gap-4">
                <div class="flex items-start gap-4">
                    ...
                    <div class="space-y-2">
                        <div class="inline-flex items-center rounded-full bg-zinc-900 px-3 py-1 text-xs font-semibold uppercase tracking-tight text-white">
                            {{ __('candidates::files.labels.archive') }}
                        </div>
                        <h3 class="text-2xl font-semibold text-zinc-900">{{ __('candidates::files.titles.all_documents') }}</h3>
        ...
        <div class="space-y-6 px-5 py-5">
            <section class="rounded-2xl border border-zinc-200 bg-white px-5 py-5 shadow-card">
                <div class="flex flex-col gap-4 border-b border-zinc-100 pb-4 xl:flex-row xl:items-start xl:justify-between">
                    <div class="flex items-start gap-4">
                        <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-900 text-white shadow-sm">
                            <x-dynamic-component :component="$draftIcon" size="w-5 h-5" color="text-white" hover="text-white" />
                        </div>
                        <div class="space-y-2">
                            <div class="flex flex-wrap items-center gap-2">
                                <h4 class="text-base font-semibold text-zinc-900">{{ __('candidates::files.titles.new_file') }}</h4>
                                <span class="inline-flex items-center rounded-full border border-zinc-200 {{ $draftTheme['badge'] }} px-2.5 py-1 text-[12px] font-semibold uppercase tracking-tight shadow-sm">
                                    {{ __('candidates::files.categories.'.($draft['category'] ?? 'other')) }}
                                </span>
                            </div>
                            <p class="max-w-3xl text-sm text-zinc-500">{{ __('candidates::files.messages.private_storage_hint') }}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <label class="group block cursor-pointer">
                                ...
                                <div class="flex min-h-[92px] items-center gap-4 rounded-[20px] border border-dashed border-zinc-300 bg-zinc-50 px-4 py-4 transition duration-200 group-hover:border-zinc-400 group-hover:bg-zinc-100/80">
                                    <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                                        ...
                                        @else
                                            <x-dynamic-component :component="$draftIcon" size="w-7 h-7" color="text-zinc-500" hover="text-zinc-600" />
                                        @endif
                                    </div>
                                    <div class="min-w-0 space-y-1">
                                        <div class="text-sm font-semibold text-zinc-800">{{ __('candidates::files.messages.upload_hint') }}</div>
                                        <div class="text-xs leading-5 text-zinc-500 break-all">{{ $uploadedFile ? $uploadedFile->getClientOriginalName() : __('candidates::files.messages.drag_drop_hint') }}</div>
                                    </div>
                                </div>
                            </label>
                            <div x-show="isUploading" class="overflow-hidden rounded-full bg-zinc-100">
                                <div class="h-2 rounded-full bg-zinc-900 transition-all duration-200" x-bind:style="`width:${progress}%`"></div>
                            </div>
                        </div>
                        @error('uploadedFile')
            ...
            <div class="rounded-2xl border border-zinc-200 bg-white shadow-card">
                <div class="flex gap-4 border-b border-zinc-100 px-5 py-5 items-center justify-between">
                  ...
                  <div class="hidden rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs font-semibold uppercase tracking-tight text-zinc-500 lg:flex lg:flex-none">
                    ...
                    @if (count($visibleFiles))
                        <div class="space-y-3">
                            @foreach($visibleFiles as $key => $file)
                                ...
                                @endphp
                                <article class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-card transition duration-200 hover:border-zinc-300 hover:shadow-card focus-within:border-zinc-400 focus-within:shadow-card">
                                    <div class="grid gap-4 px-4 py-4 xl:grid-cols-[50px,150px,minmax(0,1fr),150px,36px] xl:items-start">
                                        <div class="flex flex-col items-start gap-2">
                                            <div class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-[1rem] border border-zinc-200 bg-zinc-50 shadow-sm">
                                                ...
                                                @else
                                                    <x-dynamic-component :component="$icon" size="w-5 h-5" color="text-zinc-600" hover="text-zinc-700" />
                                                @endif
                                            </div>
                                            <div class="rounded-full border border-zinc-900 bg-black px-2.5 py-1 text-[11px] font-semibold font-mono uppercase tracking-tight text-zinc-50 shadow-sm">
                                                {{ $file['extension'] }}
                                            </div>
                                        </div>
                                        ...
                                        <div class="rounded-[20px] border border-zinc-200 bg-zinc-50 px-3 py-2 shadow-sm">
                                            <dl class="space-y-2 text-sm">
                                                <div class="space-y-1">
                                                    <dt class="text-[12px] font-semibold uppercase tracking-tight text-zinc-400">{{ __('candidates::common.labels.created_date') }}</dt>
                                                    <dd class="break-words text-zinc-700">{{ $file['created_at_label'] }}</dd>
                                                </div>
                                                <div class="space-y-1">
                                                    <dt class="text-[12px] font-semibold uppercase tracking-tight text-zinc-400">{{ __('candidates::files.labels.size') }}</dt>
                                                    <dd class="text-zinc-700">{{ \Illuminate\Support\Number::fileSize((int) $file['size_bytes']) }}</dd>
                                                </div>
                                            </dl>
                                        </div>
                                        ...
                                        <div class="flex flex-row justify-end gap-2 xl:flex-col xl:items-center">
                                            @if (!empty($file['download_url']))
                                                <a href="{{ $file['download_url'] }}"
                                                   class="flex h-10 w-10 items-center justify-center rounded-full border border-zinc-200 bg-white text-zinc-400 shadow-sm transition hover:border-zinc-300 hover:text-zinc-700"
                                                   title="{{ __('candidates::files.labels.open_file') }}">
                                                    <x-icons.arrow-icon size="w-4 h-4" color="text-zinc-400" hover="text-zinc-700"></x-icons.arrow-icon>
                                                </a>
                                            @endif
                    ...
                    @else
                        <div class="flex min-h-[260px] items-center justify-center rounded-[26px] border border-dashed border-zinc-200 bg-zinc-50/80 px-6 py-10 text-center">
                            <div class="flex max-w-sm flex-col items-center space-y-4">
                                ...
                                <div class="space-y-2">
                                    <h4 class="text-lg font-semibold text-zinc-700">{{ __('candidates::files.messages.no_files_for_filter') }}</h4>
                                    <p class="text-sm text-zinc-500">{{ __('candidates::files.messages.upload_hint') }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
            ...
            <div class="sticky bottom-0 -mx-5 border-t border-zinc-200 bg-white/95 px-5 pb-1 pt-4 backdrop-blur">
