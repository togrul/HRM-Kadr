...
class TerminationCompensationBridgeTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_termination_ends_compensation_and_reverse_reactivates_it(): void
    {
        $personnel = $this->makePersonnel('term@example.test');
        $regimeId = CompensationRegime::where('code', 'private')->value('id');
        app(CompensationService::class)->assignCompensation(
            $personnel->tabel_no,
            ['regime_id' => $regimeId, 'base_amount' => 1000, 'effective_from' => '2026-01-01'],
        );

        $this->assertSame(1, EmployeeCompensation::where('tabel_no', $personnel->tabel_no)->where('status', 'active')->count());

        $effect = app(TerminationEffect::class);

        $effect->apply(new OrderLog, ['date' => '2026-06-30'], $personnel);
        $this->assertSame(0, EmployeeCompensation::where('tabel_no', $personnel->tabel_no)->where('status', 'active')->count());
        $ended = EmployeeCompensation::where('tabel_no', $personnel->tabel_no)->first();
        $this->assertSame('ended', $ended->status);
        $this->assertSame('2026-06-30', $ended->effective_to->toDateString());

        $effect->reverse(new OrderLog, ['date' => '2026-06-30'], $personnel);
        $reactivated = EmployeeCompensation::where('tabel_no', $personnel->tabel_no)->first();
        $this->assertSame('active', $reactivated->status);
        $this->assertNull($reactivated->effective_to);
    }
