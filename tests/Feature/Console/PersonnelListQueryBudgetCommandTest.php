...
class PersonnelListQueryBudgetCommandTest extends TestCase
{
    use RefreshDatabase;

    public function test_it_reports_query_budget_metrics_for_personnel_list_flows(): void
    {
        $user = User::factory()->create();
        Permission::findOrCreate('show-personnels', 'web');
        $user->givePermissionTo('show-personnels');

        $exitCode = Artisan::call('personnel:list-query-budget', ['--json' => true]);
