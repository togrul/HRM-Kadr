...
class AttendancePermissionMatrixTest extends TestCase
{
    ...
    public function test_hr_admin_sees_all_attendance_tabs(): void
    {
        ...
        Livewire::test(Dashboard::class)
            ->assertSet('availableTabs', ['overview', 'manager-summary', 'daily-monitor', 'puantaj', 'exceptions', 'overtime', 'month-close', 'manual', 'history', 'settings', 'shifts', 'calendar-regimes'])
            ->assertSee(__('attendance::dashboard.tabs.overview'))
            ->assertSee(__('attendance::dashboard.tabs.manager_summary'))
            ...
            ->assertSee(__('attendance::dashboard.tabs.history'))
    ...
    public function test_hr_admin_tab_links_use_clean_navigate_urls(): void
    {
        $user = $this->userForRole('HR Admin');
        ...
        $response = $this->get(route('attendance', [
            ...
            'structure_id' => 55,
            'history_type' => 'shift',
            'history_subject_id' => 99,
        ]));

        $response->assertOk();
        $response->assertSee('tab=manager-summary', false);
        $response->assertSee('tab=history', false);
        $response->assertSee('year=2026', false);
        $response->assertSee('month=3', false);
        $response->assertSee('structure_id=55', false);
        $response->assertDontSee('page=2', false);
        $response->assertDontSee('history_type=shift', false);
        $response->assertDontSee('history_subject_id=99', false);
    }
    ...
    public function test_hr_manager_sees_operational_tabs_but_not_admin_shift_settings_tabs(): void
    {
        ...
        Livewire::test(Dashboard::class)
            ->assertSet('availableTabs', ['overview', 'manager-summary', 'daily-monitor', 'puantaj', 'exceptions', 'overtime', 'month-close', 'manual'])
            ->assertSee(__('attendance::dashboard.tabs.manager_summary'))
            ->assertSee(__('attendance::dashboard.tabs.manual'))
            ->assertSee(__('attendance::dashboard.tabs.exceptions'))
            ->assertSee(__('attendance::dashboard.tabs.overtime'))
            ->assertSee(__('attendance::dashboard.tabs.month_close'))
            ->assertDontSee(__('attendance::dashboard.tabs.history'))
    ...
    public function test_hr_employee_sees_module_tabs_but_not_admin_sections(): void
    {
        ...
        Livewire::test(Dashboard::class)
            ...
            ->assertSee(__('attendance::dashboard.tabs.exceptions'))
            ->assertSee(__('attendance::dashboard.tabs.overtime'))
            ->assertDontSee(__('attendance::dashboard.tabs.manager_summary'))
            ->assertDontSee(__('attendance::dashboard.tabs.history'))
    ...
    public function test_hr_auditor_can_open_module_tabs_but_not_admin_sections(): void
    {
        ...
        Livewire::test(Dashboard::class)
            ->assertSet('availableTabs', ['overview', 'daily-monitor', 'puantaj', 'exceptions', 'overtime', 'month-close', 'manual', 'history'])
            ->assertSee(__('attendance::dashboard.tabs.month_close'))
            ->assertSee(__('attendance::dashboard.tabs.manual'))
            ->assertSee(__('attendance::dashboard.tabs.exceptions'))
            ->assertSee(__('attendance::dashboard.tabs.overtime'))
