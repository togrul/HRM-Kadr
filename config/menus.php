<?php

return [
    'global' => [
        [
            'id' => 1,
            'name' => 'ui::menu.items.staff_table',
            'icon' => 'network-icon',
            'color' => 'zinc',
            'order' => 1,
            'is_active' => 1,
            'url' => 'staffs',
            'permission_id' => 1,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 2,
            'name' => 'ui::menu.items.orders',
            'icon' => 'line-order-icon',
            'color' => 'zinc',
            'order' => 2,
            'is_active' => 1,
            'url' => 'orders',
            'permission_id' => 8,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 3,
            'name' => 'ui::menu.items.personal_affairs',
            'icon' => 'personal-affair-icon',
            'color' => 'zinc',
            'order' => 3,
            'is_active' => 1,
            'url' => 'home',
            'permission_id' => 12,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 4,
            'name' => 'ui::menu.items.reports',
            'icon' => 'report-chart-icon',
            'color' => 'zinc',
            'order' => 12,
            'is_active' => 1,
            'url' => 'reports',
            'permission_id' => 3,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 5,
            'name' => 'ui::menu.items.queries',
            'icon' => 'admin-icon',
            'color' => 'zinc',
            'order' => 21,
            'is_active' => 0,
            'url' => 'home',
            'permission_id' => 3,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 7,
            'name' => 'ui::menu.items.candidates',
            'icon' => 'candidate-icon',
            'color' => 'zinc',
            'order' => 4,
            'is_active' => 1,
            'url' => 'candidates',
            'permission_id' => 17,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 8,
            'name' => 'ui::menu.items.vacations',
            'icon' => 'vacation-icon',
            'color' => 'zinc',
            'order' => 5,
            'is_active' => 1,
            'url' => 'vacations.list',
            'permission_id' => 25,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 9,
            'name' => 'ui::menu.items.business_trips',
            'icon' => 'holiday-icon',
            'color' => 'zinc',
            'order' => 6,
            'is_active' => 1,
            'url' => 'business-trips.list',
            'permission_id' => 21,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 10,
            'name' => 'ui::menu.items.time_off',
            'icon' => 'calendar-icon',
            'color' => 'zinc',
            'order' => 8,
            'is_active' => 1,
            'url' => 'leaves',
            'permission_id' => 41,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 11,
            'name' => 'ui::menu.items.my_hr',
            'icon' => 'profile-outline-icon',
            'color' => 'zinc',
            'order' => 7,
            'is_active' => 1,
            'url' => 'my-hr',
            'permission_id' => null,
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 12,
            'name' => 'ui::menu.items.attendance',
            'icon' => 'attendance-icon',
            'color' => 'zinc',
            'order' => 9,
            'is_active' => 1,
            'url' => 'attendance',
            'permission_name' => 'show-attendance',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 13,
            'name' => 'ui::menu.items.training',
            'icon' => 'training-icon',
            'color' => 'zinc',
            'order' => 10,
            'is_active' => 1,
            'url' => 'training-needs',
            'permission_name' => 'show-training-needs',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 14,
            'name' => 'ui::menu.items.performance',
            'icon' => 'performance-icon',
            'color' => 'zinc',
            'order' => 11,
            'is_active' => 1,
            'url' => 'performance-evaluation',
            'permission_name' => 'show-performance-evaluation',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 15,
            'name' => 'ui::menu.items.audit_logs',
            'icon' => 'shield-icon',
            'color' => 'zinc',
            'order' => 13,
            'is_active' => 1,
            'url' => 'audit.logs',
            'permission_name' => 'show-audit-logs',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 16,
            'name' => 'ui::menu.items.document_compliance',
            'icon' => 'document-icon',
            'color' => 'zinc',
            'order' => 14,
            'is_active' => 1,
            'url' => 'document-compliance',
            'permission_name' => 'show-document-compliance',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 17,
            'name' => 'ui::menu.items.employee_lifecycle',
            'icon' => 'refresh-icon',
            'color' => 'zinc',
            'order' => 15,
            'is_active' => 1,
            'url' => 'employee-lifecycle',
            'permission_name' => 'show-employee-lifecycle',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 18,
            'name' => 'ui::menu.items.compensation',
            'icon' => 'wallet-icon',
            'color' => 'zinc',
            'order' => 16,
            'is_active' => 1,
            'url' => 'compensation',
            'permission_name' => 'show-compensation',
            'types' => ['military', 'public', 'private'],
        ],
        [
            'id' => 19,
            'name' => 'ui::menu.items.payroll',
            'icon' => 'payroll-icon',
            'color' => 'zinc',
            'order' => 17,
            'is_active' => 1,
            'url' => 'payroll',
            'permission_name' => 'show-payroll',
            'types' => ['military', 'public', 'private'],
        ],
    ],
];
