...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.dynamic-input','data' => ['list' => $componentForms,'field' => $fieldName,'title' => $service[$_field]['title'],'type' => $_field,'model' => array_key_exists('model',$service[$_field]) ? ${$service[$_field]['model']} : null,'key' => $i,'selectedName' => array_key_exists('selectedName',$service[$_field]) ? $service[$_field]['selectedName'] : null,'searchField' => array_key_exists('searchField',$service[$_field]) ? $service[$_field]['searchField'] : null,'isCoded' => $coded_list[$i],'selectedLabel' => $resolvedLabel,'selectedValue' => $resolvedValue,'row' => $row,'disabled' => ($i+1) <= count($originalComponents)]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
