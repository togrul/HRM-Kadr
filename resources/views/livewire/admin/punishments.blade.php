        ...
        <div wire:transition class="flex border border-gray-300 rounded-md bg-slate-50 relative px-3 py-2 my-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 mt-4 w-full">
                ...
                <div class="flex flex-col">
                    ...
                    @endphp
                    <x-select-list class="w-full" :title="__('Punishment types')" mode="default" :selected="$selectedName" name="awardTypeId">
                        <x-select-list-item wire:click="setData('form','punishment_type_id',null,'---',null)" :selected="'---' ==  $selectedName"
                                            wire:model='form.punishment_type_id.id'>
                            ---
                        </x-select-list-item>
                        @foreach($punishment_types as $type)
                            <x-select-list-item wire:click="setData('form','punishment_type_id',null,'{{ trim($type->name) }}',{{ $type->id }})"
                                                :selected="$type->id === $selectedId" wire:model='form.punishment_type_id.id'>
                                {{ $type->name }}
                            </x-select-list-item>
                        @endforeach
                    </x-select-list>
                    @error('form.punishment_type_id.id')
                    <x-validation> {{ $message }} </x-validation>
                    @enderror
                </div>
                <div class="flex flex-col">
