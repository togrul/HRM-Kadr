<div style="margin-top:15px;display:grid;grid-template-columns: repeat(4, minmax(0, 1fr));align-items: center;width: 100%;border:1px solid #000;">
    ...
    <div style="display: flex;flex-direction: column;grid-column: span 3 / span 3; border-left: 1px solid #000;width: 100%;@if(count($personnel->fatherMother) > 0) min-height: 175px; padding: 5px; @endif">
        @if(count($personnel->fatherMother) > 0)
            @foreach($personnel->fatherMother as $fm)
                <div>
        ...
        @else
            @for($i = 0;$i < 7;$i++)
                <div style="@if($i < 6) border-bottom: 1px solid #000; @endif height: 25px;"></div>
            @endfor
        @endif
...
<div style="display: flex;align-items: center;">
    ...
    <span style="margin-left: 0.5rem;line-height:2;width: 35%;height: 30px;border-bottom: 1px solid #000;">{{ $personnel->idDocuments?->is_married ? 'Evli' : 'Subay' }}</span>
</div>
...
<table style="width: 100%;">
    ...
    <tbody>
        @foreach($personnel->wifeChildren as $wf)
            <tr>
                <td style="font-size: 11px;padding: 5px;">{{ $wf->fullname }}</td>
                <td style="font-size: 11px;padding: 5px;">{{ $wf->kinship->{"name_".config('app.locale')} }}</td>
                <td style="font-size: 11px;padding: 5px;">{{ \Carbon\Carbon::parse($wf->birthdate)->format('d.m.Y') }}, {{ $wf->birth_place }}</td>
                <td>
