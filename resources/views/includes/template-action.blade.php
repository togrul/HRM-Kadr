...
<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
    ...
    <div class="flex flex-col space-y-4 ">
        ...
        @error('template_data.content') <x-validation> {{ $message }} </x-validation> @enderror

        @if(array_key_exists('content',$template_data))
            @if(!is_string($template_data['content']))
                {{ $template_data['content']->getClientOriginalName() }}
            @else
                <a href="{{ asset('/storage/'.$template_data['content']) }}" target="_blank" class="text-blue-500 flex space-x-2 items-center">
