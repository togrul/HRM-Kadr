    ...
    <table {{ $attributes->merge(['class' => '-my-1 w-full caption-bottom border-separate border-spacing-y-1 text-xs max-md:flex max-md:w-full max-md:flex-col max-md:py-2']) }}>
        ...
        <tr class="group/row whitespace-nowrap md:[&amp;_td:first-child]:border-l md:[&amp;_td:last-child]:border-r md:[&amp;_td]:border-y max-md:flex max-md:w-full max-md:flex-col bg-neutral-800/50 [&amp;:hover_td]:md:!bg-neutral-800 [&amp;_td]:md:border-neutral-700/40 max-md:overflow-hidden max-md:rounded-lg max-md:border content-visibility-auto">
            @foreach ($headers as $header)
                @if($header != 'action')
                    <th class="h-10 px-2 text-left align-middle font-medium uppercase text-muted-foreground first:rounded-l-md first:pl-5 last:rounded-r-md last:pr-5 md:px-3.5 [&amp;:has([role=checkbox])]:pr-0 first:border-l last:border-r border-y border-neutral-700/40 max-md:hidden" colspan="1">
                        <span class="font-mono transition-transform duration-100 max-md:translate-x-0">{{ $header }}</span>
                    </th>
                @else
                    <th class="font-mono h-10 px-2 text-left align-middle font-medium uppercase text-muted-foreground first:rounded-l-md first:pl-5 last:rounded-r-md last:pr-5 md:px-3.5 [&amp;:has([role=checkbox])]:pr-0 border-y first:border-l last:border-r"></th>
                @endif
            @endforeach
        </tr>
        </thead>
