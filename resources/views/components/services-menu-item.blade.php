...
<button wire:click.prevent="selectService('{{ $key }}')"
    @class([
        ...
        'text-slate-800 bg-neutral-100 hover:bg-emerald-100' => $selectedService != $key,
        'text-white bg-emerald-500' => $selectedService == $key
    ])>
    <div class="flex items-center justify-center p-2 rounded-xl bg-emerald-100 text-emerald-500">
        {{ $slot }}
    </div>
