...
<x-ui.input-shell :label="$label" :error="$error" :hint="$hint" labelClass="tracking-tight text-zinc-500">
    <label for="{{ $inputId }}" class="group block cursor-pointer">
        <div class="rounded-[24px] border border-zinc-200 bg-white p-4 transition group-hover:border-zinc-300 group-hover:shadow-sm">
            <div class="flex items-start gap-4">
                <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl {{ $isSelected ? 'bg-emerald-50 text-emerald-600' : 'bg-zinc-100 text-zinc-500' }}">
                    <x-icons.document-icon class="h-5 w-5" />
                </div>
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-3">
                        <span class="inline-flex items-center rounded-2xl bg-zinc-950 px-3 py-2 text-xs font-semibold tracking-tight text-white">
