...
<div class="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-3">
    <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        @if($ready)
            <x-small-badge mode="green">{{ __('Ready to publish') }}</x-small-badge>
        @else
            <x-small-badge mode="red">{{ __('Blocked') }}</x-small-badge>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
        @foreach($checks as $check)
            <div class="flex items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2">
                ...
                @if(!empty($check['ok']))
                    <x-small-badge mode="green">{{ __('OK') }}</x-small-badge>
                @else
                    <x-small-badge mode="red">{{ __('Missing') }}</x-small-badge>
                @endif
            </div>
        @endforeach
    </div>
    ...
    @if(!$ready && !empty($blockedMessages))
        <div class="rounded-md border border-rose-200 bg-rose-50 px-3 py-2">
            <p class="text-xs font-semibold text-rose-700">{{ __('Publish guard messages') }}</p>
