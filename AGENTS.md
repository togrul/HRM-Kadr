...
- `lang/*`, `.blade.php` və ya `app/Modules/**/*.php` faylına toxunanda **hook avtomatik `translations:lint` salır**; çatışmayan açar olsa Codex-a qaytarılır (bax `.Codex/hooks/translations-lint.sh`).

## Blade / Blaze
- View-lar **blaze-safe** olmalıdır: `composer ci:blaze-safe-lint` (`--strict`) keçməlidir.
- `.blade.php` faylına toxunanda **hook avtomatik `views:blaze-safe-lint` salır**; xəta olsa Codex-a qaytarılır və düzəldilməlidir (bax `.Codex/hooks/blaze-safe-lint.sh`).
